...
const MOTION_SPEED_MULTIPLIER = 1.2
...
const INTERIOR_BLOBS: InteriorBlob[] = [
  ...
  { ox: 0.82, oy: 0.16, radius: 0.29, periodSeconds: 22, phase: 2.3, wobbleX: 0.07, wobbleY: 0.09, hueOffset: -10, alpha: 0.18 },
  { ox: 0.12, oy: 0.78, radius: 0.27, periodSeconds: 18, phase: 4.1, wobbleX: 0.08, wobbleY: 0.06, hueOffset: 12, alpha: 0.17 },
  { ox: 0.88, oy: 0.72, radius: 0.32, periodSeconds: 30, phase: 1.4, wobbleX: 0.06, wobbleY: 0.08, hueOffset: -18, alpha: 0.2 },
  { ox: 0.5, oy: 0.1, radius: 0.24, periodSeconds: 16, phase: 3.6, wobbleX: 0.1, wobbleY: 0.05, hueOffset: 18, alpha: 0.15 },
  { ox: 0.48, oy: 0.9, radius: 0.26, periodSeconds: 34, phase: 5.2, wobbleX: 0.07, wobbleY: 0.06, hueOffset: -24, alpha: 0.16 },
]

const EDGE_BLOBS: EdgeBlob[] = [
  { edge: 'top', periodSeconds: 7, phase: 0.4, travel: 0.32, anchor: 0.48, radius: 0.2, hueOffset: 4, alpha: 0.2 },
  { edge: 'bottom', periodSeconds: 12, phase: 2.5, travel: 0.34, anchor: 0.5, radius: 0.22, hueOffset: -12, alpha: 0.18 },
  { edge: 'left', periodSeconds: 9, phase: 4.4, travel: 0.26, anchor: 0.52, radius: 0.19, hueOffset: 16, alpha: 0.16 },
  { edge: 'right', periodSeconds: 18, phase: 1.7, travel: 0.28, anchor: 0.5, radius: 0.2, hueOffset: -20, alpha: 0.17 },
]
...
export function AmbientBackgroundCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = (timeMilliseconds: number) => {
      ...
      const motionTime = timeSeconds * MOTION_SPEED_MULTIPLIER

      INTERIOR_BLOBS.forEach((blob) => {
        const angle = (motionTime / blob.periodSeconds) * Math.PI * 2 + blob.phase
