...
export function DeepLearnGenerateButton({
  ...
  resourceTitle = '',
  label = 'Generate pack',
  // pendingLabel kept for API compat but unused since we no longer block
  pendingLabel: _pendingLabel = 'Queuing...',
  ...
  async function handleClick() {
    ...
    if (result.error || !result.jobId) {
      setState('error')
      dispatchInAppToast({
        title: 'Could not queue Deep Learn',
      ...
      setState('queued')
      dispatchInAppToast({
        title: 'Deep Learn added to queue.',
        description: 'Check the queue indicator above to track progress.',
        tone: 'success',
      })
  ...
  return (
    <div style={{ display: 'grid', gap: '0.35rem' }}>
      <button
        ...
        className={cn(
          className,
          (state === 'loading' || state === 'queued') && 'opacity-70 cursor-default',
        )}
      >
