    ...
    <main className="page-shell command-page">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>
            ...
            <p className="ui-page-copy" style={{ marginTop: '0.35rem', maxWidth: '46rem' }}>
              Saved exam prep packs from Deep Learn and study outputs from Do appear here, each linked back to its course, module, and source. Reopen any pack to continue where you left off.
            </p>
            {(courseFilter || moduleFilter) && (
              <div style={{ marginTop: '0.6rem' }}>
                <Link href="/library" className="ui-button ui-button-ghost ui-button-xs">
                  View all saved packs
                </Link>
              </div>
            )}
          </div>
      ...
      <section className="motion-card motion-delay-1 section-shell" style={{ padding: '1rem 1.05rem' }}>
        {availability !== 'available' ? (
          ...
          <div className="ui-empty" style={{ borderRadius: 'var(--radius-panel)', padding: '1rem', fontSize: '14px', lineHeight: 1.68 }}>
            No saved study outputs in this scope yet. Generate an exam prep pack from Learn or save a task output from Do, then come back here to continue it.
          </div>
        ) : (
            ...
            {grouped.map((group) => {
              ...
              return (
                <CourseShelf
                  ...
                  courseName={course?.name ?? 'Unassigned packs'}
